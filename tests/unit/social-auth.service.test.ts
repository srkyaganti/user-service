...
import { SocialAuthService, socialAuthService } from '../../apps/api/src/services/social-auth.service'
...
describe('SocialAuthService', () => {
  ...
  describe('getAuthorizationUrl', () => {
    it('should generate Google OAuth authorization URL', async () => {
      ...
      expect(result).toEqual({
        authorizationUrl: expect.stringContaining('accounts.google.com/o/oauth2/v2/auth'),
        state: expect.any(String),
      })
      
      expect(result.authorizationUrl).toContain('client_id=google-client-id')
      expect(result.authorizationUrl).toContain('redirect_uri=http%3A//localhost%3A3000/callback')
      expect(result.authorizationUrl).toContain('scope=openid%20email%20profile')
  ...
  describe('handleCallback', () => {
    it('should successfully handle Google OAuth callback for existing user', async () => {
      ...
      // Act
      const result = await service.handleCallback('tenant-123', 'google', callbackData)
      ...
      expect(mockCache.del).toHaveBeenCalledWith(`oauth:state:${callbackData.state}`)
    ...
    it('should throw ValidationError for invalid state token', async () => {
      ...
      // Act & Assert
      await expect(service.handleCallback('tenant-123', 'google', callbackData))
        .rejects.toThrow(ValidationError)
    })
  ...
  describe('linkAccount', () => {
    it('should successfully link social account to existing user', async () => {
      ...
      // Act
      const result = await service.linkAccount('tenant-123', 'user-123', linkData)
    ...
    it('should throw ValidationError if account already linked', async () => {
      ...
      // Act & Assert
      await expect(service.linkAccount('tenant-123', 'user-123', linkData))
        .rejects.toThrow(ValidationError)
    })
  })
  ...
  describe('unlinkAccount', () => {
    it('should successfully unlink social account', async () => {
      ...
      mockDbOperations.socialAuth.findFirst.mockResolvedValue(socialAuthToDelete)
      mockDbOperations.socialAuth.delete.mockResolvedValue(socialAuthToDelete)

      // Act
      const result = await service.unlinkAccount('tenant-123', 'user-123', 'social-123')
    ...
    it('should throw NotFoundError for non-existent social auth', async () => {
      ...
      // Act & Assert
      await expect(service.unlinkAccount('tenant-123', 'user-123', 'non-existent'))
        .rejects.toThrow(NotFoundError)
    })
    ...
    it('should throw ValidationError when trying to unlink other user\'s account', async () => {
      ...
      mockDbOperations.socialAuth.findFirst.mockResolvedValue(otherUserSocialAuth)

      // Act & Assert
      await expect(service.unlinkAccount('tenant-123', 'user-123', 'social-123'))
        .rejects.toThrow(ValidationError)
    })
  })
