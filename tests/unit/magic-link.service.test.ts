...
import {
	MagicLinkService,
	magicLinkService,
} from "../../apps/api/src/services/magic-link.service";
import {
	mockCache,
	mockDbOperations,
	mockEmailService,
...
describe("MagicLinkService", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(EmailService.getInstance).mockReturnValue(
	...
	describe("sendMagicLink", () => {
		...
		it("should throw ValidationError when registration requires invitation", async () => {
			...
			// Act & Assert
			await expect(service.sendMagicLink("tenant-123", email)).rejects.toThrow(
				ValidationError,
			);
		...
		it("should throw NotFoundError for invalid tenant", async () => {
			...
			// Act & Assert
			await expect(
				service.sendMagicLink("invalid-tenant", "test@example.com"),
		...
		it("should validate email format", async () => {
			// Act & Assert
			await expect(
				service.sendMagicLink("tenant-123", "invalid-email"),
	...
	describe("verifyMagicLink", () => {
		...
		it("should throw AuthenticationError for invalid token", async () => {
			...
			// Act & Assert
			await expect(
				service.verifyMagicLink("tenant-123", "invalid-token", {
					ipAddress: "127.0.0.1",
					userAgent: "test-agent",
				}),
		...
		it("should throw AuthenticationError for expired token", async () => {
			...
			// Act & Assert
			await expect(
				service.verifyMagicLink("tenant-123", token, {
					ipAddress: "127.0.0.1",
					userAgent: "test-agent",
				}),
			).rejects.toThrow(AuthenticationError);
	...
	describe("getMagicLinkStatus", () => {
		...
		it("should return invalid status for non-existent token", async () => {
			...
			// Act
			const result = await service.getMagicLinkStatus(
				"tenant-123",
				"invalid-token",
			);
	...
	describe("revokeMagicLink", () => {
		...
		it("should return success even for non-existent token", async () => {
			...
			// Act
			const result = await service.revokeMagicLink(
				"tenant-123",
				"non-existent-token",
			);
	...
	describe("listActiveMagicLinks", () => {
		it("should return active magic links for user", async () => {
			...
			const mockPatternScan = vi
				.fn()
				.mockResolvedValue(["magic-link:token-1", "magic-link:token-2"]);
			mockCache.scan = mockPatternScan;

			// Act
			const result = await service.listActiveMagicLinks(
				"tenant-123",
				"user-123",
			);
	...
	describe("revokeAllMagicLinks", () => {
		it("should successfully revoke all magic links for user", async () => {
			...
			// Act
			const result = await service.revokeAllMagicLinks(
				"tenant-123",
				"user-123",
			);
