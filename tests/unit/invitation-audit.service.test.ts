...
describe("InvitationService", () => {
	...
	describe("sendInvitation", () => {
		...
		it("should throw ConflictError for existing pending invitation", async () => {
			...
			mockDbOperations.invitation.findFirst.mockResolvedValue(
				existingInvitation,
			);
	...
	describe("getInvitation", () => {
		...
		it("should throw NotFoundError for non-existent invitation", async () => {
			...
			// Act & Assert
			await expect(
				invitationService.getInvitation("invalid-token"),
		...
		it("should throw ValidationError for expired invitation", async () => {
			...
			// Act & Assert
			await expect(
				invitationService.getInvitation("expired-token"),
		...
		it("should throw ValidationError for already accepted invitation", async () => {
			...
			// Act & Assert
			await expect(
				invitationService.getInvitation("accepted-token"),
			).rejects.toThrow(ValidationError);
		});
	});
...
describe("AuditService", () => {
	...
	describe("exportAuditLogs", () => {
		...
		it("should throw ValidationError for unsupported format", async () => {
			// Act & Assert
			await expect(
				auditService.exportAuditLogs("tenant-123", {
					format: "xml" as any,
				}),
			).rejects.toThrow(ValidationError);
		});
