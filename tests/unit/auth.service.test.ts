...
import { mockDbOperations, mockCache, mockEvents, mockUser } from '../helpers/test-utils'
...
describe('AuthService', () => {
  ...
  describe('login', () => {
    it('should successfully login with valid credentials', async () => {
      ...
      expect(verifyPassword).toHaveBeenCalledWith(mockLoginData.password, mockUser.passwordHash)
    ...
    it('should throw AuthenticationError for non-existent user', async () => {
      ...
      // Act & Assert
      await expect(authService.login('tenant-123', mockLoginData))
    ...
    it('should throw NotFoundError for invalid tenant', async () => {
      ...
      // Act & Assert
      await expect(authService.login('invalid-tenant', mockLoginData))
  ...
  describe('register', () => {
    ...
    it('should throw ValidationError for existing user', async () => {
      ...
      // Act & Assert
      await expect(authService.register('tenant-123', mockRegisterData))
    ...
    it('should throw ValidationError for expired invitation', async () => {
      ...
      // Act & Assert
      await expect(authService.register('tenant-123', registerDataWithInvitation))
        .rejects.toThrow(ValidationError)
    })
    ...
    it('should throw ValidationError when organization is required but not provided', async () => {
      ...
      vi.mocked(dbManager.getTenant).mockResolvedValue(tenantRequiringOrg as any)
      mockDbOperations.user.findUnique.mockResolvedValue(null)
  ...
  describe('verifyMFA', () => {
    ...
    it('should throw AuthenticationError for invalid MFA token', async () => {
      ...
      // Act & Assert
      await expect(authService.verifyMFA('invalid-token', {
        type: 'TOTP',
        code: '123456',
        ipAddress: '127.0.0.1',
        userAgent: 'test-agent',
