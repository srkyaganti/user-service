...
describe('UserActivationService', () => {
  ...
  beforeEach(() => {
    ...
    EmailService.prototype.sendActivationEmail = mockEmailService.sendActivationEmail
    TenantSettingsService.prototype.getSettings = mockTenantSettingsService.getSettings
  ...
  describe('sendActivationEmail', () => {
    it('should send activation email and store token', async () => {
      ...
      await service.sendActivationEmail('tenant-123', 'user-123', 'test@example.com')
      ...
      expect(mockEmailService.sendActivationEmail).toHaveBeenCalledWith(
        'test@example.com',
        {
          activationUrl: 'http://localhost:3000/activate?token=test-activation-token'
        }
      )
  ...
  describe('activateAccount', () => {
    ...
    it('should activate account with valid token', async () => {
      ...
      expect(mockCacheService.delete).toHaveBeenCalledWith('activation:test-activation-token')
      expect(mockAuditService.log).toHaveBeenCalledWith('tenant-123', {
        userId: 'user-123',
  ...
  describe('resendActivation', () => {
    it('should resend activation for inactive user', async () => {
      ...
      const result = await service.resendActivation('tenant-123', 'test@example.com')
    ...
    it('should handle non-existent user', async () => {
      ...
      const result = await service.resendActivation('tenant-123', 'nonexistent@example.com')
