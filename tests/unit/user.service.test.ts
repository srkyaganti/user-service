...
describe("UserService", () => {
	...
	describe("getProfile", () => {
		...
		it("should throw NotFoundError for non-existent user", async () => {
			...
			// Act & Assert
			await expect(
				userService.getProfile("tenant-123", "non-existent"),
	...
	describe("updateProfile", () => {
		...
		it("should validate website URL format", async () => {
			...
			// Act & Assert
			await expect(
				userService.updateProfile("tenant-123", "user-123", invalidUpdateData),
	...
	describe("changePassword", () => {
		...
		it("should throw ValidationError for incorrect current password", async () => {
			...
			// Act & Assert
			await expect(
				userService.changePassword("tenant-123", "user-123", passwordData),
			).rejects.toThrow(ValidationError);
	...
	describe("uploadAvatar", () => {
		...
		it("should throw ValidationError for invalid file type", async () => {
			...
			// Act & Assert
			await expect(
				userService.uploadAvatar("tenant-123", "user-123", invalidFile as any),
			).rejects.toThrow(ValidationError);
		...
		it("should throw ValidationError for file too large", async () => {
			...
			// Act & Assert
			await expect(
				userService.uploadAvatar("tenant-123", "user-123", largeFile as any),
			).rejects.toThrow(ValidationError);
		});
	});
	...
	describe("searchUsers", () => {
		it("should successfully search users by query", async () => {
			...
			// Act
			const result = await userService.searchUsers("tenant-123", searchQuery, {
				limit: 20,
				offset: 0,
			});
		...
		it("should apply pagination correctly", async () => {
			...
			// Act
			await userService.searchUsers("tenant-123", "test", {
				limit: 10,
				offset: 30,
			});
	...
	describe("changeEmail", () => {
		...
		it("should throw ConflictError for email already in use", async () => {
			...
			// Act & Assert
			await expect(
				userService.changeEmail("tenant-123", "user-123", newEmail),
			).rejects.toThrow(ValidationError);
		});
	...
	describe("deleteAccount", () => {
		...
		it("should throw ValidationError for incorrect password", async () => {
			...
			// Act & Assert
			await expect(
				userService.deleteAccount("tenant-123", "user-123", passwordData),
			).rejects.toThrow(ValidationError);
