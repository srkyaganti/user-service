...
export class TestDatabaseManager {
	...
	async setupTestEnvironment(): Promise<void> {
		...
		process.env.DATABASE_URL =
			"postgresql://postgres:password@localhost:5433/userservice_test";
		...
		for (let attempt = 1; attempt <= maxAttempts; attempt++) {
			try {
				...
				if (attempt === maxAttempts) {
					throw new Error(
						`Test services not ready after ${maxAttempts} attempts: ${error}`,
					);
				}

				console.log(
					`Attempt ${attempt}/${maxAttempts}: Services not ready, waiting...`,
				);
	...
	private async checkPostgresReady(): Promise<void> {
		try {
			execSync(
				"docker exec user-service-postgres-test pg_isready -U postgres -d userservice_test",
				{
					stdio: "pipe",
				},
			);
	...
	private async initializeTestDatabase(): Promise<void> {
		try {
			...
			execSync(
				'DATABASE_URL="postgresql://postgres:password@localhost:5433/userservice_test" bun run --cwd packages/database push --force-reset',
				{
					stdio: "pipe",
					cwd: process.cwd(),
				},
	...
	private async initializeTenantDatabase(tenantId: string): Promise<void> {
		try {
			...
			console.error(
				`Failed to initialize tenant database for ${tenantId}:`,
				error,
			);
			throw error;
		}
