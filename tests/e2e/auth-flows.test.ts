...
import { createTestTenant, createTestOrganization } from '../setup'
...
describe('End-to-End Authentication Flows', () => {
  ...
  describe('Organization and Team Flow', () => {
    it('should complete organization creation and team management flow', async () => {
      ...
      const teamResponse = await client.api.v1.organizations[':orgId'].teams.$post({
        param: { orgId },
        header: authHeaders,
        json: {
      ...
      const invitationResponse = await client.api.v1.organizations[':orgId'].invitations.$post({
        param: { orgId },
        header: authHeaders,
        json: {
      ...
      const membersResponse = await client.api.v1.organizations[':orgId'].members.$get({
        param: { orgId },
        header: authHeaders,
      })
      ...
      const teamsResponse = await client.api.v1.organizations[':orgId'].teams.$get({
        param: { orgId },
        header: authHeaders,
      })
  ...
  describe('Device Management Flow', () => {
    it('should complete device registration and management flow', async () => {
      ...
      const trustResponse = await client.api.v1.devices[':deviceId'].trust.$post({
        param: { deviceId },
        header: authHeaders,
      })
      ...
      const deviceDetailsResponse = await client.api.v1.devices[':deviceId'].$get({
        param: { deviceId },
        header: authHeaders,
      })
  ...
  describe('Session Management Flow', () => {
    it('should complete session management flow', async () => {
      ...
      const revokeAllResponse = await client.api.v1.sessions['revoke-all'].$post({
        header: authHeaders,
        json: {
          exceptCurrent: true,
        },
  ...
  describe('Audit Trail Flow', () => {
    it('should track user activities in audit logs', async () => {
      ...
      const registrationLog = logs.find((log: any) => log.action === 'user.register')
      const profileUpdateLog = logs.find((log: any) => log.action === 'user.update.profile')
      const orgCreationLog = logs.find((log: any) => log.action === 'organization.create')
