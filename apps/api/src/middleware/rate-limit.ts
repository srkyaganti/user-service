...
import { getEnvVarAsInt, getClientIp, RateLimitError } from '@user-service/shared'
...
export async function rateLimitMiddleware(c: Context, next: Next) {
  ...
  try {
    ...
    if (error instanceof Error && 'msBeforeNext' in error) {
      ...
      c.header('X-RateLimit-Remaining', ((error as any).remainingPoints || 0).toString())
      c.header('X-RateLimit-Reset', new Date(Date.now() + (error as any).msBeforeNext).toISOString())
