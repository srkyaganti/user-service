...
import { AuthenticationError, AuthorizationError, AuthUser, CACHE_KEYS } from '@user-service/shared'
...
async function checkUserPermission(user: AuthUser, permission: string): Promise<boolean> {
  if (!user.organizationId) {
    return false
  }
