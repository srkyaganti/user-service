...
import type { Organization, User } from "@user-service/database";
import {
	AuthenticationError,
	CACHE_KEYS,
	EVENTS,
...
export class AuthService {
	 ...
	 * @param data - Login credentials with IP and user agent info
	 * @returns User object with authentication tokens
	 * @throws AuthenticationError if credentials are invalid
	 * @throws ValidationError if login method is disabled
	 */
	async login(
		tenantId: string,
		data: LoginDto & { ipAddress: string; userAgent: string },
	) {
		...
		const isEmailPasswordEnabled = await tenantSettings.isLoginMethodEnabled(
			tenantId,
			"emailPassword",
		);
		if (!isEmailPasswordEnabled) {
			throw new AuthenticationError(
				"Email/password login is disabled for this tenant",
			);
		}
		...
		const isMfaRequired = await tenantSettings.isMfaRequired(
			tenantId,
			user.isTenantAdmin,
		);
	 ...
	 * @param tenantId - The tenant identifier
	 * @param data - Registration data with IP and user agent info
	 * @returns Created user object with authentication tokens
	 * @throws ValidationError if registration is disabled or data is invalid
	 * @throws AuthenticationError if user already exists
	 */
	async register(
		tenantId: string,
		data: RegisterDto & { ipAddress: string; userAgent: string },
	) {
		const db = await dbManager.getClient(tenantId);
		const tenant = await dbManager.getTenant({ id: tenantId });
		...
		if (
			...
			throw new ValidationError(
				"Organization membership required for B2B tenants",
			);
		}

		// Validate password against tenant policy
		const passwordValidation = await tenantSettings.validatePassword(
			tenantId,
			data.password,
		);
		...
		const requiresActivation =
			await tenantSettings.isActivationRequired(tenantId);
