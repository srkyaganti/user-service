...
export class DeviceService {
	...
	async trustDevice(deviceId: string, userId: string, tenantId: string) {
		const db = await dbManager.getClient(tenantId);
	...
	async removeDevice(deviceId: string, userId: string, tenantId: string) {
		const db = await dbManager.getClient(tenantId);
	...
	async logoutDevice(deviceId: string, userId: string, tenantId: string) {
		const db = await dbManager.getClient(tenantId);
	...
	async getDeviceStats(userId: string, tenantId: string) {
		...
		const [totalDevices, trustedDevices, activeSessions, deviceTypes] =
			await Promise.all([
				db.device.count({
					where: { userId },
				}),
		...
		return {
			...
			trustedDevices,
			activeSessions,
			deviceTypes: deviceTypes.reduce(
				(acc, item) => {
