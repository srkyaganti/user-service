...
export class EventService {
  ...
  async publish(eventName: string, data: any, options?: {
    delay?: number
    attempts?: number
  }) {
    ...
    const worker = this.getWorker('events', async (job) => {
      if (job.name === eventName) {
        try {
          ...
          logger.error({ error, eventName, jobId: job.id }, 'Event processing failed')
          throw error
        }
      }
    })
  ...
  private getWorker(name: string, processor: (job: any) => Promise<void>): Worker {
