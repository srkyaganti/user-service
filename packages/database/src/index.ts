...
// Global PrismaClient instance with logging
...
export class TenantDatabaseManager {
  ...
  async getClient(tenantId: string): Promise<PrismaClient> {
    if (!this.connections.has(tenantId)) {
      ...
      const databaseUrl = process.env.NODE_ENV === 'production' && tenant.dbHost
  ...
  async updateTenantStatus(tenantId: string, status: 'ACTIVE' | 'SUSPENDED' | 'TRIAL' | 'ARCHIVED') {
